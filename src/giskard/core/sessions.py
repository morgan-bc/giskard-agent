...
from .middleware import ChatContext, ChatMiddleware
...
# activates a ``_RunPersistenceGate`` around the guarded section of a run;
...
_DEFERRED_RUN_PERSISTENCE: ContextVar[_RunPersistenceGate | None] = ContextVar(
    "giskard_deferred_run_persistence", default=None
)
...
_CURRENT_RUN_IDENTITY: ContextVar[object | None] = ContextVar("giskard_current_run_identity", default=None)
...
_PENDING_GATE_CLAIM: ContextVar[tuple[_RunPersistenceGate | None, object] | None] = ContextVar(
    "giskard_pending_run_persistence_gate_claim", default=None
)
...
class _RunPersistenceGate:
    ...
    Usage (the gate owner is egress-enforcement middleware)::
...
LOCAL_HISTORY_CONVERSATION_ID = "giskard_local_history_persistence"
