...
def _apply_get_response_docstrings() -> None:
    ...
    try:
        from .middleware import ChatMiddlewareLayer
    except ImportError as exc:
        # When ``clients`` is imported as a side effect of ``middleware``'s own
        # module-level ``from .clients import SupportsChatGetResponse``, this
        # late docstring alignment would re-enter a partially initialized
        # ``middleware``. Skip alignment in that case; whichever module is
        # imported second applies the layered docstrings.
        middleware_module = __name__.rsplit(".", 1)[0] + ".middleware"
        if exc.name == middleware_module and "partially initialized module" in str(exc):
            return
        raise

    from .tools import FunctionInvocationLayer
