    ...
    *,
    ...
    Keyword Args:
        ...
        tool_approval_rule: Optional approval preset. ``"yolo"`` auto-approves
            ...
            non-destructive shell commands); write-tool ``file_name`` arguments
            must resolve inside the workdir boundary (paths resolving outside
            it escalate), deletion operations (``delete_file``,
            ``file_memory_delete``, destructive shell commands such as
            ``rm``/``Remove-Item``) and any unknown tool still require human
            approval. Requires ``disable_tool_auto_approval=False``. Known
            limitation: the shell check inspects each command segment's leading
            token only, so ``sudo rm`` / ``xargs rm`` / aliases are not caught
            — approval is a UX boundary, not a sandbox.
