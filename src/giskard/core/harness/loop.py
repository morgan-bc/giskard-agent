...
This module provides :class:`AgentLoopMiddleware`, an :class:`~giskard.AgentMiddleware`
...
1. A user-supplied ``should_continue`` predicate - for example, keep looping while a response does
   not yet contain a completion marker, while a :class:`~giskard.TodoProvider` still has
   open items, or while a :class:`~giskard.BackgroundAgentsProvider` still has running
...
from ..middleware import AgentContext, AgentMiddleware, MiddlewareTermination
from ..types import (
    AgentResponse,
    AgentResponseUpdate,
    AgentRunInputs,
...
class AgentLoopMiddleware(AgentMiddleware):
    ...
    By default a non-streaming run returns an aggregated :class:`~giskard.AgentResponse`
    ...
    Examples:
        .. code-block:: python
            ...
            from giskard import Agent, AgentResponse
            from giskard.harness._loop import AgentLoopMiddleware
        ...
        *,
        ...
        Keyword Args:
            ...
            return_final_only: Controls what a non-streaming run returns. When ``False`` (default),
                the returned :class:`~giskard.AgentResponse` aggregates every iteration: each
                ...
                only the final iteration's :class:`~giskard.AgentResponse` is returned. This
...
def _running_background_tasks(session: Any, agent: Any) -> list[Any]:
    ...
    Resolves the :class:`~giskard.BackgroundAgentsProvider` from the running agent
    ...
    """
    from .background_agents import BackgroundAgentsProvider, BackgroundTaskInfo, BackgroundTaskStatus
...
def background_tasks_running() -> ShouldContinueCallable:
    ...
    This resolves the :class:`~giskard.BackgroundAgentsProvider` from the running agent
...
def background_tasks_running_message(*, session: Any = None, agent: Any = None, **kwargs: Any) -> str | None:
    ...
    :class:`~giskard.BackgroundAgentsProvider` from the agent, lists the still-running tasks,
...
def _resolve_context_provider(agent: Any, provider_type: type) -> Any:
    ...
    :func:`~giskard.create_harness_agent` wired up instead of constructing their own.
    """
    return next(
...
def todos_remaining(*, looping_modes: Sequence[str] | None = None) -> ShouldContinueCallable:
    ...
    This resolves the :class:`~giskard.TodoProvider` from the running agent
    (``agent.context_providers``) rather than taking it as an argument, so it can be used directly
    with :func:`~giskard.create_harness_agent` (whose providers are built internally) as well
    ...
    Args:
        looping_modes: When provided, the loop only continues while the agent's current operating
            mode (read from its :class:`~giskard.AgentModeProvider`) is one of these modes;
    ...
    async def _should_continue(*, session: Any = None, agent: Any = None, **kwargs: Any) -> bool:
        from .mode import AgentModeProvider, get_agent_mode
...
async def todos_remaining_message(*, session: Any = None, agent: Any = None, **kwargs: Any) -> str | None:
    ...
    :class:`~giskard.TodoProvider` from the agent, lists the still-open todo items, and
