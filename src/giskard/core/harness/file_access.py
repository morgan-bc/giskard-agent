...
``read_file``, ``edit_file``, ``glob``, and ``grep`` by default, plus the
...
from ..sessions import AgentSession, ContextProvider, SessionContext
from ..telemetry import FeatureIndex, mark_feature_used
from ..tools import ApprovalMode, tool
...
DEFAULT_FILE_ACCESS_INSTRUCTIONS = (
    "## File Access\n"
    "You have access to a shared file storage area via the `read_file`, `write_file`, "
    "`edit_file`, `glob`, and `grep` tools for reading, writing, and managing files.\n"
    ...
    "asked you to do so.\n"
    "- Files may be organized into subdirectories. Use `glob` to find files by "
    "name pattern, "
    "or `grep` to search file contents recursively across "
    "the whole store."
)
...
class _LineEdit(BaseModel):
    """A single literal line replacement for ``edit_file_lines``."""
...
class _ReplaceLinesInput(BaseModel):
    """Input schema for ``edit_file_lines``."""
...
class _GlobInput(BaseModel):
    """Input schema for ``glob``."""

    pattern: Annotated[
        str,
        Field(
            description=(
                'Glob pattern matched against each file\'s path relative to `directory` (e.g. "*.md" matches '
                'markdown files at any depth, "reports/*" matches everything under reports). fnmatch semantics: '
...
class FileAccessProvider(ContextProvider):
    ...
    - ``write_file`` — Write a file (refuses to overwrite by default).
    - ``read_file`` — Read the content of a file by name.
    - ``edit_file`` — Replace occurrences of a substring within a file.
    - ``glob`` — Recursively find files whose paths match a glob pattern.
    - ``grep`` — Recursively search file contents using a
      case-insensitive regex, optionally filtered by a glob pattern and base directory.

    Optional tools (advertised only when ``enable_extra_tools`` is set):

    - ``ls`` — List the direct child files and subdirectories of a
      directory, optionally filtered by a glob pattern.
    - ``delete_file`` — Delete a file by name.
    - ``edit_file_lines`` — Replace whole lines within a file.
    ...
    ``disable_readonly_tool_approval`` (read_file, ls, glob, grep) and/or
    ...
    - :meth:`read_only_tools_auto_approval_rule` — auto-approves only the
      read-only tools (read_file, ls, glob, grep), while still prompting for the
      tools that modify the store (write_file, delete_file, edit_file,
      edit_file_lines).
        ...
        *,
        ...
        Keyword Args:
            ...
            disable_write_tools: When ``True``, only the read-only tools
                (``read_file``, ``glob``, ``grep``, and the optional ``ls``)
                are advertised; the write tools (``write_file``,
                ``delete_file``, ``edit_file``, ``edit_file_lines``) are hidden
                from the model.
            enable_extra_tools: When ``True``, the optional tools (``ls``,
                ``delete_file``, and ``edit_file_lines``) are advertised in
                addition to the default tools. Defaults to ``False``.
        ...
        self.enable_extra_tools = enable_extra_tools
    ...
    def read_only_tools_auto_approval_rule(function_call: Content) -> bool:
        ...
        .. warning::
            **Security — avoid tool-name collisions.** This rule approves local
            tool calls by tool name only (``read_file``,
            `ls``, ``glob``, and ``grep``). Any other local tool
    ...
    def all_tools_auto_approval_rule(function_call: Content) -> bool:
        ...
        .. warning::
            **Security — avoid tool-name collisions.** This rule approves local
            tool calls by tool name only (``write_file``,
            ``read_file``, ``delete_file``, ``ls``,
            ``glob``, ``grep``, ``edit_file``, and
            ``edit_file_lines``). Any other local tool registered under
    ...
    async def before_run(
        ...
        *,
        ...
        async def write_file(file_name: str, content: str, overwrite: bool = False) -> str:
        ...
        async def edit_file_lines(file_name: str, edits: list[_LineEdit]) -> str:
        ...
        async def grep(
            ...
            (directly usable with read_file), along with snippets and matching lines with line numbers.
            ...
            # root so the names compose directly with read_file/edit_file/delete_file.
            prefix = target.strip("/")
        ...
        @tool(name=FileAccessProvider.GLOB_TOOL_NAME, schema=_GlobInput, approval_mode=readonly_approval)
        async def glob(pattern: str, directory: str | None = None) -> list[str] | str:
            """Find files in the store whose paths match a glob pattern.

            The search runs recursively across all subdirectories. Optionally restrict the search
            to a ``directory`` (relative path). The pattern is matched against each file's path
            relative to ``directory`` using fnmatch semantics where ``*`` matches any characters
            including ``/``: use ``"*.md"`` to match markdown files at any depth, or
            ``"reports/*"`` to match everything under the ``reports`` subtree.
            Returns the list of matching file paths relative to the store root (directly usable
            with read_file), sorted lexicographically.
            """
            if not pattern or not pattern.strip():
                return "Could not search files: pattern must not be empty."
            target = directory if directory and directory.strip() else ""
            try:
                return await self._glob_files(target, pattern)
            except ValueError as exc:
                return f"Could not search files: {exc}"
            except OSError as exc:
                return f"Could not search files: {exc.strerror or exc}"
        ...
        if self.enable_extra_tools:
            tools.append(ls)
            if not self.disable_write_tools:
                tools.extend([delete_file, edit_file_lines])
        context.extend_tools(self.source_id, tools)

    async def _glob_files(self, directory: str, pattern: str) -> list[str]:
        """Recursively collect file paths under ``directory`` whose relative path matches ``pattern``.

        Walks the store via :meth:`AgentFileStore.list_children`, so symlink/reparse-point
        protections of the concrete store apply. Returns paths relative to the store root
        (forward slashes), sorted lexicographically.
        """
        matches: list[str] = []
        stack: list[tuple[str, str]] = [(directory, "")]
        while stack:
            current_dir, rel_prefix = stack.pop()
            children = await self.store.list_children(current_dir)
            subdirectories: list[tuple[str, str]] = []
            for entry in children:
                rel = f"{rel_prefix}/{entry.name}" if rel_prefix else entry.name
                if entry.type == FileStoreEntry.DIRECTORY:
                    child_dir = f"{current_dir}/{entry.name}" if current_dir else entry.name
                    subdirectories.append((child_dir, rel))
                elif _matches_glob(rel, pattern):
                    matches.append(rel)
