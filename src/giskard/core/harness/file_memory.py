...
File access is mediated through the :class:`~giskard.AgentFileStore`
abstraction (shared with :class:`~giskard.FileAccessProvider`), so the
same in-memory, local-disk, or remote-blob backends can be reused here.

Unlike :class:`~giskard.FileAccessProvider`, which exposes a *shared*
...
per-invocation :class:`~giskard.SessionContext` in
